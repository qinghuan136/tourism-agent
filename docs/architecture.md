...
当前采用北京对话小样本评测后人工选定的参数：候选数 20、Rerank 分数阈值 0.81、
语义去重余弦阈值 0.98。自动加载最多返回 3 条，工具调用仍默认最多返回 5 条；
评测中的最多 3 条指标不直接代表工具调用效果。0.81 偏向减少误召回，并非通用最优阈值，
后续扩大语料后需复测。修改环境变量后需要重启服务器。
